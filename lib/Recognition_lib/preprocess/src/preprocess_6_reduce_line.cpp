...
/*
    ...
    {
        if( (src_bin.at<uchar>(y0-5,x0-5) == 0) || (src_bin.at<uchar>(y0+5,x0+5) == 0)  ) ;//do nothing
        else
        {
        src_bin.at<uchar>(y0  +go*one_step*one_step_height , x0+go*one_step) = 255;
        src_bin.at<uchar>(y0-1+go*one_step*one_step_height , x0+go*one_step) = 255;
        src_bin.at<uchar>(y0+1+go*one_step*one_step_height , x0+go*one_step) = 255;
        src_bin.at<uchar>(y0-2+go*one_step*one_step_height , x0+go*one_step) = 255;
        src_bin.at<uchar>(y0+2+go*one_step*one_step_height , x0+go*one_step) = 255;
        src_bin.at<uchar>(y0-3+go*one_step*one_step_height , x0+go*one_step) = 255;
        src_bin.at<uchar>(y0+3+go*one_step*one_step_height , x0+go*one_step) = 255;
...
*/
    ...
    {
/*
        if( (src_bin.at<uchar>(y0-5,x0) == 255) || (src_bin.at<uchar>(y0+5,x0) == 255) ||
            (src_bin.at<uchar>(y0-4,x0) == 255) || (src_bin.at<uchar>(y0+4,x0) == 255) ||
            (src_bin.at<uchar>(y0-3,x0) == 255) || (src_bin.at<uchar>(y0+3,x0) == 255) ||
            (src_bin.at<uchar>(y0-2,x0) == 255) || (src_bin.at<uchar>(y0+2,x0) == 255) )
*/
/*
        if( (src_bin.at<uchar>(y0-1+go*one_step*one_step_height , x0+go*one_step) == 255) ||
            (src_bin.at<uchar>(y0+1+go*one_step*one_step_height , x0+go*one_step) == 255) )
        {
            src_bin.at<uchar>(y0  +go*one_step*one_step_height , x0+go*one_step) = 255;
        ...
        if( (src_bin.at<uchar>(y0-2+go*one_step*one_step_height , x0+go*one_step) == 255) && (src_bin.at<uchar>(y0-1+go*one_step*one_step_height , x0+go*one_step) == 0) )
        {
            src_bin.at<uchar>(y0  +go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-1+go*one_step*one_step_height , x0+go*one_step) = 255;
        }else ;//do nothing
        ...
        if( (src_bin.at<uchar>(y0-3+go*one_step*one_step_height , x0+go*one_step) == 255) && (src_bin.at<uchar>(y0-2+go*one_step*one_step_height , x0+go*one_step) == 0) )
        {
            src_bin.at<uchar>(y0  +go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-1+go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-2+go*one_step*one_step_height , x0+go*one_step) = 255;
        ...
        if( (src_bin.at<uchar>(y0-4+go*one_step*one_step_height , x0+go*one_step) == 255) && (src_bin.at<uchar>(y0-3+go*one_step*one_step_height , x0+go*one_step) == 0) )
        {
            src_bin.at<uchar>(y0  +go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-1+go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-2+go*one_step*one_step_height , x0+go*one_step) = 255;
        ...
        if( (src_bin.at<uchar>(y0-5+go*one_step*one_step_height , x0+go*one_step) == 255) && (src_bin.at<uchar>(y0-4+go*one_step*one_step_height , x0+go*one_step) == 0) )
        {
            src_bin.at<uchar>(y0  +go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-1+go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-2+go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-3+go*one_step*one_step_height , x0+go*one_step) = 255;
            src_bin.at<uchar>(y0-4+go*one_step*one_step_height , x0+go*one_step) = 255;
...
/**///		cout<<"i = "<<i<<" ,";
    ...
    //			one_step = 1;//(i * 1 / b ) * b; //往左走了 -rho * cos 個 sin
                Erase_line(src_bin,x0,y0,one_step,one_step_height,1);
                x0 += one_step;
                y0 += one_step * one_step_height;
    ...
    //二、X0 Y0 不會被更新喔喔喔喔喔喔！
                ...
                {
                        if( Check_shift(src_bin, x0+i*one_step , y0 +i*one_step*one_step_height +0 ,one_step,one_step_height) != CHECK_FAILED )
                        {
    ...
    //(就那個空格來測測看像不像線)，同時也希望跳躍空洞
                            //往右10格
                            if(src_bin.at<uchar>(y0,x0+i*one_step) != 0)
                            {
                                for(int i = 1 ; i < CHECK_LINE_LENGTH * CODA_RATE ; i++)
                                {
                                    //先 丟下一格要走的點 來預測他是不是線~~~ 如果是才移動喔！！
                                    if(   Check_shift(src_bin, x0+i*one_step , y0 +i*one_step*one_step_height +0,one_step,one_step_height ) != CHECK_FAILED
                                       && src_bin.at<uchar>(y0 +i*one_step*one_step_height +0 ,x0+i*one_step) == 0 )
                                    {

                                        Erase_line(src_bin,x0,y0,one_step,one_step_height,i);
    ...
    //反正就是不會有出現 往右跳完以後，還會在往上下跳的狀況！因為往右跳，代表已經在線上，在線上下面的if就會擋掉了！
                            ...
                            {
                                //多加這個是希望他往比較 "像線"的地方跑
                                int cmp_up   = Check_shift(src_bin, x0+i*one_step , y0 + i*one_step*one_step_height -1 ,one_step,one_step_height);
                                int cmp_down = Check_shift(src_bin, x0+i*one_step , y0 + i*one_step*one_step_height +1 ,one_step,one_step_height);

                                //會多加 == 0 ，是希望他跑得更嚴謹、精確不要亂亂跑
                                if(cmp_up >= cmp_down && cmp_up != CHECK_FAILED && src_bin.at<uchar>(y0 +i*one_step*one_step_height -1 ,x0+i*one_step) == 0)
                                ...
                                else if(cmp_down > cmp_up && cmp_down != CHECK_FAILED && src_bin.at<uchar>(y0 +i*one_step*one_step_height +1 ,x0+i*one_step) == 0 )//if( cmp_up != CHECK_FAILED && cmp_down == CHECK_FAILED)
                                {
                                    Erase_line(src_bin,x0,y0,one_step,one_step_height,i);
                                    for(int go = 0 ; go < i ; go++)
                                        {
...
//cout<<"bend~~~"<<endl;
    ...
    ////////////////// Range防呆 ///////////////////
                        //多加這個是希望他往比較 "像線"的地方跑
                        int cmp_up   = Check_shift(src_bin, x0,y0-i,one_step,one_step_height);
                        int cmp_down = Check_shift(src_bin, x0,y0+i,one_step,one_step_height);
    ...
    /////////////////////// 如果上下測完仍然還是沒辦法~~~就print出來，表示也許不是線maybe已經找到頭囉！///////////////////
                    if(src_bin.at<uchar>(y0,x0) != 0 && Check_shift(src_bin, x0,y0,one_step,one_step_height) ==  CHECK_FAILED)// && error >= ok_error2)
                    {
                        break;
                    }
                }	////////////////////////////////////
...
///******************************************************
	...
	imwrite(window_name + "_g_img.bmp",src_bin);
