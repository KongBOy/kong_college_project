...

int* Recognition_page::Distance_detect(){
    debuging = debuging_pre4;
    // 找線之間距離的層級

    // 先排好rho再微分 來看 rho的變化量 即 distance, 之後再對distance排序 即可看出線之間的距離有哪些種類 , preprocess_3 找出 lines 已經有對 rho 做排序了 這邊就不用排囉
    int * rho = new int[lines.size()];
    for(int i = 0 ; i < lines.size() ; i++) rho[i] = lines[i][0];

    int * dist = new int[lines.size()-1];
    for(int i = 0 ; i < lines.size() -1 ; i++) dist[i] = rho[i+1] - rho[i];
    bubbleSort(lines.size()-1, dist);

    // distance先排好之後再微分(上面已排) 來看 distance的變化量, 可看出不同distance層級之間的變化
    int * dist_diffi_1 = new int[lines.size()-1-1];
    for(int i = 0 ; i < lines.size() -1 -1; i++) dist_diffi_1[i] = dist[i+1] - dist[i];
    if(debuging){
        for(int i = 0 ; i < lines.size() -1 -1; i++) 
            cout << "rho[" << setw(2) << i                  << "]=" << setw(4) << rho[i]                   << " , dist(sorted, the order is not related to rho)[" << setw(2)  << i                  << "] = " << setw(4) << dist[i]                  << " , dist_diffi_1[" << setw(2) << i << "] = " << setw(4) << dist_diffi_1[i] << endl;
        cout     << "rho[" << setw(2) << lines.size() -1 -1 << "]=" << setw(4) << rho[lines.size() -1 -1]  << " , dist(sorted, the order is not related to rho)[" << setw(2)  << lines.size() -1 -1 << "] = " << setw(4) << dist[lines.size() -1 -1] << endl;
        cout     << "rho[" << setw(2) << lines.size() -1    << "]=" << setw(4) << rho[lines.size() -1   ]  << setw(2) << endl;
        cout     << endl;
        // 直接看結果比較好理解
