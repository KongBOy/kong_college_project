...
void Perspective_trans(double x, double y, Mat warp_matrix, double& result_x, double& result_y, bool debuging){
    // 參考網站:
...
void Recognition_page::Cut_staff()
               // trans_start_point的意思是 五線譜最左上角的那一點
{
    ...
    for(int staff_num = 0 ; staff_num < staff_count ; staff_num++){
        ...
        if( left_min [staff_num] - left_max[staff_num]  > 30 || 
            ...
            if(debuging) cout << "wrong staff" << endl;
            staff_count--;
            continue;
        }
        ...
        if(debuging) for(int i = 0 ; i < 4 ; i++) cout << "src4P[" << i << "].x = " << src4P[i].x << " src4P[" << i << "].y = " << src4P[i].y << endl;
        ...
        if(debuging) for(int i = 0 ; i < 4 ; i++) cout << "dst4P[" << i << "].x = " << dst4P[i].x << " dst4P[" << i << "].y = " << dst4P[i].y << endl;

        // 取得轉換矩陣
        Mat warp_matrix = getPerspectiveTransform(src4P, dst4P);
        if(debuging) cout << "warp_matrix:" << endl << warp_matrix << endl;
