...
// 把 轉正 和 二值化後的影像 裡面的白點 全部移到左邊形成直條圖 會形成像山一樣的形狀, 找出 由五線譜形成的山 並 在該區間的影像找出五線譜的線
void Recognition_page::Horizon_map_to_find_line(){
    debuging = debuging_pre3b;
    // src_bin_roi    : 轉正 和 二值化後的影像
    // horizontal_img : src_bin_roi 一樣大小的 全黑 影像, 來裝 水平投影的容器
    Mat horizontal_img(src_bin_roi.rows ,src_bin_roi.cols, CV_8UC1, Scalar(0));
    const int width = src_bin_roi.cols - THRESH_HOLD;
    const int distance_range_min = src_bin_roi.rows / (STAFF_COUNT_MAX*2);
    const int distance_range_max = src_bin_roi.rows / (STAFF_COUNT_MIN*2);
    ...
    if(debuging){
        cout << "src_bin_roi.cols=" << src_bin_roi.cols << endl;
    ...
    // 建立容器
    int * w_count = new int[src_bin_roi.rows]; // src_bin_roi 的每一個 row 有幾個 pixel 是白色的
    int e_count;             // 山的數量
    int l_edge[100];         // 山的左邊
    int r_edge[100];         // 山的右邊
    ...
    // 初始化容器
    for(int i = 0 ; i < src_bin_roi.rows ; i++) w_count[i] = 0;
    for(int i = 0 ; i < 100 ; i++){
        l_edge[i] = 0;
        r_edge[i] = 0;
        distance[i] = 0;
    ...
    // 走訪 src_bin_roi 的 每一個row, 找出 山的 左, 右側, row, 面積
    for(int go_row = 0 ; go_row < src_bin_roi.rows; go_row++){
        for(int go_col = 0 ; go_col < src_bin_roi.cols ; go_col++){
            if(src_bin_roi.at<uchar>(go_row,go_col) == 255) horizontal_img.at<uchar>(go_row, w_count[go_row]++) = 255;
        }
        ...
        if(go_row > 0){
            ...
            // **       本row
                ...
                if(debuging){
                    imshow      ("all_mountain", src_bin_roi);
                    cvMoveWindow("all_mountain",  10, 10);
                    imshow      ("mountain"    , src_bin_roi(Rect(THRESH_HOLD, l_edge[e_count],  width, distance[e_count])));
    ...
    if(debuging) See_mountain(src_bin_roi, e_count, l_edge, r_edge, distance, mountain_area, "debug_img", "pre3_HorizL_2_1_mountain_1_before");
    ...
    if(debuging) See_mountain(src_bin_roi, e_count, l_edge, r_edge, distance, mountain_area, "debug_img", "pre3_HorizL_2_1_mountain_2_after");


    // 找出五線譜的線
    for(int go_m = 0 ; go_m < e_count ; go_m++){
        Mat proc_img = src_bin_roi(Rect(0,l_edge[go_m], width, distance[go_m]));

        vector<Vec2f> lines_local;
        ...
        while(lines_local.size() < 5 && hough_w_th_ratio >= hough_w_th_ratio_low_bound){
            HoughLines(proc_img, lines_local, 1 , CV_PI/180, src_bin_roi.cols * hough_w_th_ratio / 100 , 0  , 0 );
            hough_w_th_ratio -= 3;
            ...
            if(debuging) {
                ...
                Watch_Hough_Line(lines_local, proc_img, "", watch_hough_process_name);
            }
        }

        // 如果這座山有找到線, 丟進去 lines 容器內
        if(lines_local.size() > 0){
            // cout<<"find_line!!!"<<endl;
            for(int i = 0 ; i < lines_local.size() ; i++){
                Vec2f temp_data2;
                temp_data2[0] = (int)(l_edge[go_m] * sin(lines_local[i][1]) + lines_local[i][0]);
                temp_data2[1] = lines_local[i][1];
                lines.push_back(temp_data2);
            }
        }
    ...
    if(debuging) {
        Watch_Hough_Line(lines, src_bin_roi, "", (string)"debug_img/" + "pre3_HorizL_2_2_All_line");
