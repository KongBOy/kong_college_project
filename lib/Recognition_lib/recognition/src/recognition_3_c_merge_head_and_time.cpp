...
/// 合併成真的NOTE囉
    ...
    for(int go_head = 0 ; go_head < maybe_head_count ; go_head++)
    {
        ...
        ///    <<" , lines[0]["  <<go_line<<"] = "<<lines[0][go_line]
        ///    <<" , value = "   <<maybe_head[0][go_head] - lines[0][go_line];
        ...
        line( debug_img , Point(lines[0][go_line],lines[1][go_line]) , Point(lines[0][go_line], lines[1][go_line] + lines[2][go_line]) , Scalar(100,200,0) , 2 );
...
///        waitKey(0);
        ...
        int distance = maybe_head[0][go_head] - lines[0][go_line]; /// 值代表大小，+-代表方向，-往左，+往右
        /// 左邊的離線太遠 不抓線，直接存
        if((maybe_head[0][go_head] - lines[0][go_line]) < ((template_img.cols + dist_error) * -1) )
        {
            ///cout註解 標記現在在哪個case
            ///cout<<" , case 1";
        ...
        /// 和線夠近 抓 同一條 線
        else if( ((maybe_head[0][go_head] - lines[0][go_line]) >= ((template_img.cols + dist_error) * -1)) &&
                 ((maybe_head[0][go_head] - lines[0][go_line]) <= (0 + dist_error) ) )
        {
            ///cout註解 標記現在在哪個case
            ///cout<<" , case 2";
            ...
            {
                if(go_line >= lines_count-1) ;///如果是在最後一條線出現此case的話，就一定是併排的那種拉，防呆 ~~~do nothing~~~
                else
                {
                    ///cout註解 標記現在在哪個case
                    ///cout<<endl<<"embarass~~case "<<endl;

                    int distance_next = maybe_head[0][go_head] - lines[0][go_line+1];
            ...
            note[1][note_count] =  maybe_head[1][go_head];
            note[2][note_count] =  head_type;
            note[3][note_count] =  lines_time[go_line]; /// 抓同條線
            note_count++;
        }
        ///右邊的離線太遠  換 下一條線 ， 這顆頭 換了下一條線 後 需要再跟 這條新的線 比一次，所以head--
        else /// 即if((maybe_head[0][go_head] - lines[0][go_line] > (0 + dist_error))
        {
            ///cout註解 標記現在在哪個case
            ///cout<<" , case 3";
            go_line++;
