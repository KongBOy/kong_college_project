...

void Recognition_staff_img::Grab_MaybeHead_from_ResultMap(Mat template_img, Mat target_img, Mat staff_result_map, float thresh_hold){
    debuging = debuging_recog1;
    Mat temp_show;
    cvtColor(target_img, temp_show, CV_GRAY2BGR);
    ...
    // 二、找出框框內最好的點，
    // 三、只留下那個點其他點去掉，
    MaybeHead_MergeCloseHead(staff_result_map, target_img, template_img, debuging);
...
void Recognition_staff_img::recognition_1_find_all_MaybeHead(Mat template_img, Mat target_img, Mat& staff_result_map, string method){
    debuging = debuging_recog1;
    ...
    int cur_template_result_map_row = target_img.rows - template_img.rows+1;
    int cur_template_result_map_col = target_img.cols - template_img.cols+1;
    ...
    int extend_full;
    int extend_half;
    int right_boundary_index = target_img.cols - 1;
    // 走訪每座山
    for(int go_mountain = 0; go_mountain < e_count; go_mountain++){
        ...
        if(debuging) cout << "target_img.cols:" << target_img.cols << ", template_img.cols:" << template_img.cols << ", grab_left:" << grab_left << ", grab_width:" << grab_width << endl;
        ...
        Mat proc_part_img = target_img(Rect(grab_left, 0, grab_width, target_img.rows ));

        // 三前置、 建立 放 根據垂直投影切出來的影像做樣本比對結果的容器，根據垂直投影找出來的mountain切, 所以容器大小是: 山圖的大小 - template大小 + 1
        int result_row = target_img.rows - template_img.rows +1;
