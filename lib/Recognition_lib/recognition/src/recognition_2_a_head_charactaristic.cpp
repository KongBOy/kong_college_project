...
void recognition_2_a_head_charactristic(int head_type, Mat template_img, Mat staff_bin_erase_line, Mat cut_ord_img, int& maybe_head_count, float maybe_head[][200], bool debuging){
    Mat debug_img;
    cvtColor(staff_bin_erase_line, debug_img, CV_GRAY2BGR);
    // 用明顯特徵過濾以前 先顯示一下目前結果, 等等做完後會再顯示一次可以來比較
    if(debuging){
        cv::imshow  ("template_img", template_img);
        cvMoveWindow("template_img", 10, 10);
        ...
        // 怕太亂 重新整理debug_img 回 staff_bin_erase_line
        cvtColor(staff_bin_erase_line, debug_img, CV_GRAY2BGR);
    }
    ...
    for(int go_head = 0 ; go_head < maybe_head_count ; go_head++){
        ...
        switch(head_type){
            ...
            case 4:{
                ...
                for(int go_y = check_left_top.y ; go_y < check_left_top.y + check_size ; go_y++){
                    for(int go_x = check_left_top.x ; go_x < check_left_top.x + check_size ; go_x++){
                        if(staff_bin_erase_line.at<uchar>(go_y,go_x)){
                            empty_inside = true;
            ...
            case 3:{  // 二分休止 在未消線的圖中 以自身往外延伸 的左下 且 右下 都有黑色線, 如果此特徵不夠多納就排除 是 二分休止
                ...
                float l_black_rate = black_count_function(cut_ord_img, l_top, l_down, l_left, l_right, debug_img);
                float r_black_rate = black_count_function(cut_ord_img, r_top, r_down, r_left, r_right, debug_img);
            ...
            case 8:{
                ...
                float ru_black_rate = black_count_function(staff_bin_erase_line, ru_top, ru_down, ru_left, ru_right, debug_img);
                float ld_black_rate = black_count_function(staff_bin_erase_line, ld_top, ld_down, ld_left, ld_right, debug_img);
